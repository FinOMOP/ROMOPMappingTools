...
# Lab values files format

## quantity_source_unit_conversion.tsv file format

The `quantity_source_unit_conversion.tsv` file is used to describe the quantity source unit conversion.
It is a TSV file with the following columns:

| Name                     | Type      | Description                                                | Rules    |
| ------------------------ | --------- | ---------------------------------------------------------- | -------- |
| omop_quantity            | character | The OMOP quantity concept name                             | not empty, must be in the Vocabulary |
| source_unit_valid        | character | The source unit to convert from                            | not empty, must be in the validUnitsList |
| to_source_unit_valid     | character | The target unit to convert to                              | not empty, must be in the validUnitsList |
| conversion               | character | The conversion formula or factor                           | not empty, must be a valid conversion formula |
| only_to_omop_concepts    | integer   | Applicable to this OMOP concepts only                      | optional, must exist in the CONCEPT table|

omop_quantity, source_unit_valid and to_source_unit_valid must be unique combinations.

## Extra Usagi rules for lab values

The following rules are applied to the Usagi file when the pathToValidUnitsFile and pathToUnitConversionFile are provided:

ERROR:
- "LAB: Invalid lab source name format": The sourceName must be of the format {testName}[{testUnit}]
- "LAB: APPROVED Invalid lab unit": All approved mappings must have a testUnit that is in the provided units usagi file
- "LAB: APPROVED Invalid lab quantity": All approved mappings must have a testUnit quantity that agrees with the quantity in the vocabularies for the mapped concept

WARNING:
- "LAB: Invalid lab mapped domain": If the domain of the mapped concept is not 'Measurement'
- "LAB: not APPROVED Invalid lab unit": An not approved mappings has a testUnit that is in the provided units usagi file
- "LAB: not APPROVED Invalid lab quantity": An not approved mappings has a testUnit quantity that does not agree with the quantity in the vocabularies for the mapped concept
