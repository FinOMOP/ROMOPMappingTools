...
```{r}
vocabularyFolderToSTCMAndVocabularyTables(pathToVocabularyFolder, connection, vocabularyDatabaseSchema, sourceToConceptMapTable)
```
...
# Validate the CDM tables with Data Quality Dashboard
...
resultsDQD <- validateCDMtablesWithDQD(connectionDetails, vocabularyDatabaseSchema)
```

```{r}
resultsDQDjson <- tempfile(fileext = ".json")
jsonlite::write_json(resultsDQD, resultsDQDjson)
...
```{r}
dplyr::tbl(connection, "RELATIONSHIP") |>
    dplyr::collect()
